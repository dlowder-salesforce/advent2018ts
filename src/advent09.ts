...
const provideInput = (): string => {
  return fs.readFileSync('input/input09.txt', 'utf8').trim();
};
...
const playGame = (nPlayers: number, lastMarbleScore: number): number => {
  const scores: number[] = [];
  let i: number;
  for (i = 0; i < nPlayers; i++) {
    scores.push(0);
  }
  const marbleCircle: CircularList = new CircularList();
  marbleCircle.insertAfterCurrent(0);
  let currentPlayer: number = 0;
  let marbleToInsert: number = 0;
  while (marbleToInsert < lastMarbleScore) {
    marbleToInsert++;
    if (marbleToInsert % 23 === 0) {
      scores[currentPlayer] += marbleToInsert;
      marbleCircle.moveCurrent(-7);
      scores[currentPlayer] += marbleCircle.currentValue();
      marbleCircle.moveCurrent(-1);
      marbleCircle.removeAfterCurrent();
      marbleCircle.moveCurrent(1);
    } else {
      marbleCircle.moveCurrent(1);
      marbleCircle.insertAfterCurrent(marbleToInsert);
      marbleCircle.moveCurrent(1);
    }
...
const advent09 = {
  part1: (): number => {
    ...
    return playGame(nPlayers, lastMarbleScore);
  },
