...
const provideInput = (): number[] => {
  return fs
    .readFileSync('input/input08.txt', 'utf8')
    .trim()
    .split(' ')
    .map(s => parseInt(s, 10));
};

const processNextNode = (input: number[]): Node => {
  const node: Node = new Node();
  const nodeCount = input.shift();
  const metadataCount = input.shift();
  let i: number;
  for (i = 0; i < nodeCount; i++) {
    node.children.push(processNextNode(input));
  }
...
const metadataSum = (node: Node): number => {
  let total: number = node.metadata.reduce((a, b) => a + b, 0);
...
const nodeValue = (node: Node): number => {
  ...
  for (const m of node.metadata) {
    if (m <= node.children.length) {
      total += nodeValue(node.children[m - 1]);
    }
  }
  return total;
};
...
const advent08 = {
  part1: (): number => {
    const input: number[] = provideInput();
    const root: Node = processNextNode(input);
