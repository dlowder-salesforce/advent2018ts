...
const provideInput = (): string => {
  return fs.readFileSync('input/input05.txt', 'utf8').trim();
};

// tslint:disable-next-line:max-line-length
const replaceAllPairs = /aA|Aa|bB|Bb|cC|Cc|dD|Dd|eE|Ee|fF|Ff|gG|Gg|hH|Hh|iI|Ii|jJ|Jj|kK|Kk|lL|Ll|mM|Mm|nN|Nn|oO|Oo|pP|Pp|qQ|Qq|rR|Rr|sS|Ss|tT|Tt|uU|Uu|vV|Vv|wW|Ww|xX|Xx|yY|Yy|zZ|Zz/g;

const replaceEachPair = [
  /a|A/g,
  /b|B/g,
  /c|C/g,
  /d|D/g,
...
const react = (s: string): string => {
  while (replaceAllPairs.exec(s)) {
    s = s.replace(replaceAllPairs, '');
  }
  return s;
};
...
const advent05 = {
  part1: () => {
    let s: string = provideInput();
    s = react(s);
    return s.length;
  },

  part2: () => {
    const s: string = react(provideInput());
    let min: number = s.length;
    for (const re of replaceEachPair) {
      let r: string = s.replace(re, '');
      r = react(r);
      if (r.length < min) {
        min = r.length;
      }
